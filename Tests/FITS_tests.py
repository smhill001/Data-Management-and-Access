...
for molecule in files:
    ...
    show2=axs1[2].imshow(np.flip(datanewNH3[0,:,:],axis=0)/dataref,'coolwarm',vmin=0.95,vmax=1.05)
    pl.colorbar(show2, 
...
fig2,axs2=pl.subplots(3,1,figsize=(4.0,6.0), dpi=150, facecolor="white",sharex=True)
fig2.suptitle('new/new')

filename=pthnew+files['NH3']["new"]
print(filename)
hdulist=fits.open(filename)
hdulist.info()
hdr=hdulist[0].header
...
filename=pthnew+files['CH4']["new"]
hdulist=fits.open(filename)
hdulist.info()
hdr=hdulist[0].header
datanewCH4=hdulist[0].data
#fNH3sza=fNH3hdulist[1].data
#fNH3eza=fNH3hdulist[2].data
hdulist.close()

show2=axs2[0].imshow(np.flip(datanewNH3[0,:,:],axis=0),'gray',vmin=0.85,vmax=1.05)
pl.colorbar(show1, 
               orientation='vertical',cmap='gray',
               ax=axs2[0],fraction=0.046, pad=0.04)
axs2[0].set_title("new NH3")

show2=axs2[1].imshow(np.flip(datanewCH4[0,:,:],axis=0),'gray',vmin=0.85,vmax=1.05)
pl.colorbar(show2, 
               orientation='vertical',cmap='gray',
               ax=axs2[1],fraction=0.046, pad=0.04)
axs2[1].set_title("new CH4")

show2=axs2[2].imshow(np.flip(datanewCH4[0,:,:],axis=0)/np.flip(datanewNH3[0,:,:],axis=0),'gray',vmin=0.95,vmax=1.05)
pl.colorbar(show2, 
               orientation='vertical',cmap='coolwarm',
               ax=axs2[2],fraction=0.046, pad=0.04)
axs2[2].set_title("new CH4")
