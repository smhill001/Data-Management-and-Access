...
For each observation, the pipeline outputs:
  ...
  - L1
    ...
    - Scientific Files: Creates new fits file with incidence, emission, and radiance values averaged between the two files
  - L2: 
...
The process can be called on a directory with a name of the format YYYYMMDDUT. The path to the parent directory should be listed in the config.py under the 'input' keyword. The process is run by calling the batch_process function on the directory name. For example:

```
batch_process("20251016UT")
```

A popup will appear in which you should rotate and scale the image of Jupiter to fit the required dimensions before closing it. Once the process runs, a new directory will be created with the same name as the input directory in the output directory listed in the config file. If this directory exists, its contents will be overwritten . An example file structure for this directory is:
...
A test mode can be set in the config file, which will set the input path in the batch process script to Test_Data/Inputs and output the results to Test_Data/New_Results
